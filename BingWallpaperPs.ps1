...
if (Test-Path $FHD_filepath) {
    ...
    $FHD = "true" # Means, the FHD file has been downloaded.
}
...
if (Test-Path $UHD_filepath) {
    ...
    $UHD = "true" # Means, the FHD file has been downloaded.
}
...
if (Test-Path $LNK_filepath) {
    ...
    $LNK = "true" # Means, the FHD file has been saved.
...
if ($IsWindows) {
    ...
    if ($FHD -eq "true") {
        # There is no way that the wall paper is applied 
    ...
    Add-Content -Path notification.ps1 -Encoding utf8 -Value "    `$Notifier = [Windows.UI.Notifications.ToastNotificationManager]::CreateToastNotifier(`"BingWallpaperPs`")"
    ...
    # FTT = THE PICTURES WAS SAVED AND THE WALLPAPER IS NOW UPDATED 
    ...
    if (($LNK -eq "false") -and ($UHD -eq "false") -and ($FHD -eq "false")) {
        # Add-Content -Path notification.ps1 -Encoding utf8 -Value "Show-Notification `"Nothing updated or downloaded.`""
        Write-Output "Everything is already saved. Nothing to do."
    }

    if (($LNK -eq "false") -and ($UHD -eq "false") -and ($FHD -eq "true" )) {
        Add-Content -Path notification.ps1 -Encoding utf8 -Value "Show-Notification `"The FHD picture was saved and the wallpaper is now updated.`""
    }

    if (($LNK -eq "false") -and ($UHD -eq "true") -and ($FHD -eq "false")) {
        Add-Content -Path notification.ps1 -Encoding utf8 -Value "Show-Notification `"The UHD picture was saved.`""
    }

    if (($LNK -eq "false") -and ($UHD -eq "true") -and ($FHD -eq "true")) {
        Add-Content -Path notification.ps1 -Encoding utf8 -Value "Show-Notification `"The pictures were saved and the wallpaper is now updated.`""
    }

    if (($LNK -eq "true") -and ($UHD -eq "false") -and ($FHD -eq "false")) {
        Add-Content -Path notification.ps1 -Encoding utf8 -Value "Show-Notification `"The LNK file was saved.`""
    }

    if (($LNK -eq "true") -and ($UHD -eq "false") -and ($FHD -eq "true")) {
        Add-Content -Path notification.ps1 -Encoding utf8 -Value "Show-Notification `"The LNK file and FHD picture was saved and wallpaper is now updated.`""
    }

    if (($LNK -eq "true") -and ($UHD -eq "true") -and ($FHD -eq "false")) {
        Add-Content -Path notification.ps1 -Encoding utf8 -Value "Show-Notification `"The LNK file and UHD picture was saved.`""
    }

    if (($LNK -eq "true") -and ($UHD -eq "true") -and ($FHD -eq "true")) {
        Add-Content -Path notification.ps1 -Encoding utf8 -Value "Show-Notification `"Today's Bing wallpaper was saved and the wallpaper is now updated.`""
    }
